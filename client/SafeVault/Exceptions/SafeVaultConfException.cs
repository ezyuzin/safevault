...
{
    public class SafeVaultConfException : Exception
    {
        public SafeVaultConfException(Exception inner, string message, params object[] args)
            : base(args.Length == 0 ? message : string.Format(message, args), inner)
        {
        }

        public SafeVaultConfException(string message, params object[] args)
