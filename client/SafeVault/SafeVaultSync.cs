    ...
    public sealed class SafeVaultSync
    {
            ...
            SafeVaultConf vaultConf)
        {
            ...
            if (lastModified != null)
            {
                ...
                vaultConf.SyncRemoteLastModified = lastModified;
                vaultConf.Save();
        ...
        private void SyncUpload(PwDatabase pwDatabase, SafeVaultWebClient webClient, string databaseUuid,
            SafeVaultConf vaultConf)
        {
            var lastModified = GetLastModified(pwDatabase).ToString("u");
        ...
        private void SyncLocal(PwDatabase pwDatabase, SafeVaultWebClient webClient, string databaseUuid, SafeVaultConf vaultConf)
        {
            ...
            if (lastModified == vaultConf.SyncRemoteLastModified)
            {
        ...
        {
            SetStatusText("Check changes...");
            var localLastModified = GetLastModified(pwDatabase).ToString("u");
            var lastModified = GetRemoteLastModified(webClient, databaseUuid);
            ...
            if (lastModified != null && lastModified != vaultConf.SyncRemoteLastModified)
            {
                SetStatusText("Downloading...");
                var dbData = Async.Invoke(() => DownloadFile(webClient, databaseUuid, vaultConf));
                if (dbData != null)
                {
                    ...
                    _host.MainWindow.Enabled = false;
                    localLastModified = GetLastModified(pwDatabase).ToString("u");
                }
            }
            ...
            vaultConf.SyncRemoteLastModified = localLastModified;
            vaultConf.Save();
            pwDatabase.Save(new NullStatusLogger());
        }
        ...
        private static string GetRemoteLastModified(SafeVaultWebClient webClient, string databaseUuid)
        {
            try
            {
                return Async.Invoke(
                     () => webClient.GetDbLastModified(databaseUuid));
            }
            catch (SafeVaultHttpException httpEx)
            {
        ...
        private byte[] DownloadFile(SafeVaultWebClient webClient, string databaseUuid, SafeVaultConf vaultConf)
        {
            ...
            try
            {
                databaseData = webClient.Download(databaseUuid,
            ...
            catch (SafeVaultHttpException httpEx)
            {
                if (httpEx.StatusCode == HttpStatusCode.NotFound)
                {
                    throw new SafeVaultSyncException("Database not found on SafeVault. Please sync with SafeVault first.");
                }
                throw;
            }
            return databaseData;
        ...
        private void SyncDatabase(PwDatabase pwDatabase, byte[] dbData, bool forceSave)
        {
            ...
            try
            {
                ...
                if (!success.HasValue)
                {
                    throw new SafeVaultSyncException(
                        "Synchronization failed.\n\nYou do not have permission to import. Adjust your KeePass configuration.");
                }
                if (!(bool)success)
                    throw new SafeVaultSyncException("Synchronization failed.\n\n" +
